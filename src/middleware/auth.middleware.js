...
const verifyPermission=async(ctx,next)=>{
    ...
    /* 34-6-3.❗如何动态获取tabeleName?---思路2:我们接口的写法都是严格按照momentId,commentId等来写的,
                把Id截取掉就是我们的表名moment,comment。所以可以通过以下做法获取tableName 
                获取{commentId:'1'} */
    const [resourceKey]=Object.keys(ctx.params)//commentId,Object.keys获取的是字段
    const tableName=resourceKey.replace('Id','')//comment 
    const resourceId=ctx.params[resourceKey]//1
    //34-7.❗将28-5-5的momentId注释掉,将34-5的momentId改成resourceId
    ...
    try{//用try catch的写法可以捕获异常,避免未授权--是上面的严谨写法
        //28-5-10.查询是否具备权限
            //34-5.❗将checkMoment改成checkResource,并传入一个tableName
...
/*
//34-6-1.❗如何动态获取tabeleName?---思路1:使用闭包的方法
